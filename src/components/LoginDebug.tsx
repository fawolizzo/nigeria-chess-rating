...
const LoginDebug = () => {
  ...
  const currentUserFromStorage = getFromStorageSync<User>(
    'ncr_current_user',
    null
  );
    ...
    <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-4">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div>
          ...
          {allUsers
            .filter((u) => u.role === 'rating_officer')
            .map((officer, idx) => (
              <div
                key={idx}
                className="ml-2 mb-1 p-1 border-b border-gray-200 dark:border-gray-700"
              >
                <div>
                  <span className="text-blue-500">Email:</span> {officer.email}
                </div>
                <div>
                ...
                <div>
                  <span className="text-blue-500">Has Password:</span>{' '}
                  {officer.password ? 'Yes' : 'No'}
                </div>
                <div>
                  <span className="text-blue-500">Access Code:</span>{' '}
                  {officer.accessCode || 'RNCR25'}
                </div>
        ...
        <div>
          ...
          {allUsers.filter((u) => u.role === 'tournament_organizer').length ===
            0 && (
            <div className="text-red-500 ml-2">
              No tournament organizers found!
            </div>
          )}
          {allUsers
            .filter((u) => u.role === 'tournament_organizer')
            .map((organizer, idx) => (
              <div
                key={idx}
                className="ml-2 mb-1 p-1 border-b border-gray-200 dark:border-gray-700"
