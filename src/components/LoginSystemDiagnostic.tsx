...
const LoginSystemDiagnostic: React.FC = () => {
  ...
  useEffect(() => {
    const ratingOfficers = Array.isArray(users)
      ? users.filter((u) => u.role === 'rating_officer')
      : [];
    const approvedRatingOfficers = Array.isArray(ratingOfficers)
      ? ratingOfficers.filter((u) => u.status === 'approved')
      : [];
    const pendingRatingOfficers = Array.isArray(ratingOfficers)
      ? ratingOfficers.filter((u) => u.status === 'pending')
      : [];

    const tournamentOrganizers = Array.isArray(users)
      ? users.filter((u) => u.role === 'tournament_organizer')
      : [];
    const approvedTournamentOrganizers = Array.isArray(tournamentOrganizers)
      ? tournamentOrganizers.filter((u) => u.status === 'approved')
      : [];
    const pendingTournamentOrganizers = Array.isArray(tournamentOrganizers)
      ? tournamentOrganizers.filter((u) => u.status === 'pending')
      : [];
        ...
        <AccordionItem value="accounts">
          ...
          <AccordionContent>
            <div className="space-y-3 text-xs">
              ...
              {users.filter((user) => user.role === 'rating_officer').length ===
                0 && (
