...
const PerformanceChart: React.FC<PerformanceChartProps> = ({
  ...
  const initialRating =
    chartData.length > 0 ? chartData[0].rating : currentRating;
  const highestRating =
    chartData.length > 0
  ...
  const minRating = Math.max(
    Math.floor((lowestRating - 50) / 100) * 100,
    FLOOR_RATING
  ); // Never go below floor rating
          ...
          <>
            <div className="flex justify-between mb-4 text-sm">
              <div>
                ...
                <div
                  className={netChange >= 0 ? 'text-green-500' : 'text-red-500'}
                >
                  {netChange > 0 ? '+' : ''}
                  {netChange}
                </div>
