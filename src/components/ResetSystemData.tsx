...
const ResetSystemData: React.FC<ResetSystemDataProps> = ({ onReset }) => {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      logMessage(LogLevel.WARNING, 'ResetSystemData', "User initiated system reset");
      ...
      if (currentUserEmail && 
         (currentUserEmail.includes('test') || 
          ...
          window.confirm(`Do you want to delete the Supabase auth user "${currentUserEmail}"? This will allow you to reuse this email for testing.`))) {
        
        console.log(`Attempting to delete Supabase auth user: ${currentUserEmail}`);
      ...
      toast({
        ...
        description: "All system data has been cleared. You will be redirected to the homepage.",
      });
      ...
      logMessage(LogLevel.ERROR, 'ResetSystemData', "System reset failed", error);
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md text-sm text-red-700 dark:text-red-300">
          <p>Warning: All data will be permanently deleted. You will be logged out 
          and redirected to the homepage.</p>
          
          <p className="mt-2">
            <strong>Note:</strong> This will also attempt to delete the auth user from Supabase 
            if it's a test account, allowing you to register with the same email again.
          </p>
        </div>
