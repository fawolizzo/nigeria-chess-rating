...
const SystemTestRunner = () => {
  ...
  const [results, setResults] = useState<{
    hasRun: boolean;
    success: boolean;
    logMessages: { message: string; type: 'info' | 'error' | 'success' | 'warning' }[];
  }>({
    hasRun: false,
    success: false,
    logMessages: []
  ...
  const runTests = async () => {
    ...
    const logMessages: { message: string; type: 'info' | 'error' | 'success' | 'warning' }[] = [];
    ...
    try {
      ...
      const hasErrors = logMessages.some(log => 
        log.type === 'error' || log.message.includes('FAILED') || log.message.includes('failed')
      );
  ...
  return (
    <Card className="w-full shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Run comprehensive system tests to verify all components are working together correctly
        </CardDescription>
      </CardHeader>
