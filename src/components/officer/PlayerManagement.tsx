...
const PlayerManagement: React.FC<PlayerManagementProps> = ({
  ...
  const handlePlayersImported = async (importedPlayers: Partial<Player>[]) => {
    console.log(
      '🚀 PlayerManagement: handlePlayersImported called with',
    ...
    for (let i = 0; i < importedPlayers.length; i++) {
      ...
      console.log(
        `🔄 PlayerManagement: Processing player ${i + 1}/${importedPlayers.length}:`,
        playerData.name
      );
      ...
      if (playerData.name && playerData.email) {
        try {
          ...
          addedCount++;
          console.log(
            `✅ PlayerManagement: Successfully created player ${playerData.name}`
          );
        } catch (error) {
          errorCount++;
    ...
    console.log(
      `📊 PlayerManagement: Import completed. Added: ${addedCount}, Errors: ${errorCount}`
    );
    ...
    try {
      ...
      console.error(
        '❌ PlayerManagement: Error refreshing player list:',
        error
      );
    }
    ...
    if (addedCount > 0 && errorCount === 0) {
      ...
      errors.forEach((error) =>
        console.error('❌ PlayerManagement: Import error:', error)
      );
    } else {
      toast({
        title: '❌ Upload Failed',
        description:
          'No players were imported. Please check your file and try again.',
        variant: 'destructive',
      });
  ...
  const pendingCount = (Array.isArray(players) ? players : []).filter(
    (p) => p.status === 'pending'
  ).length;
  const approvedCount = (Array.isArray(players) ? players : []).filter(
    (p) => p.status === 'approved'
  ).length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Dialog
            open={isUploadPlayersOpen}
            onOpenChange={setIsUploadPlayersOpen}
          >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewPlayer({ ...newPlayer, fideId: e.target.value })
                    }
                ...
                <Button
                  ...
                  className="w-full bg-nigeria-green hover:bg-nigeria-green-dark"
                >
                  Create Player
                </Button>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      {player.state && <p>State: {player.state}</p>}
