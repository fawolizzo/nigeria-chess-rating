...
const MultiFormatRatingChart = ({
  player,
  height = 350,
}: MultiFormatRatingChartProps) => {
  ...
  const chartData = useMemo(() => {
    ...
    sortedDates.forEach((date) => {
      ...
      const rapidEntry = player.rapidRatingHistory?.find(
        (r) => r.date === date
      );
      const blitzEntry = player.blitzRatingHistory?.find(
        (r) => r.date === date
      );

      const reason =
        classicalEntry?.reason || rapidEntry?.reason || blitzEntry?.reason;
      ...
      <CardContent>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                ...
                stroke="#9ca3af"
              />
              <YAxis domain={yDomain} stroke="#9ca3af" />
              ...
              {player.rapidRatingHistory &&
                player.rapidRatingHistory.length > 0 && (
                  <Line
                    type="monotone"
                    dataKey="rapid"
                    name="Rapid"
              ...
              {player.blitzRatingHistory &&
                player.blitzRatingHistory.length > 0 && (
                  <Line
                    type="monotone"
                    dataKey="blitz"
                    name="Blitz"
        ...
        <div className="text-xs text-gray-500 mt-4 px-1">
          ...
          <ul className="list-disc list-outside ml-4 space-y-1">
            <li>
              Floor rating of {FLOOR_RATING} for new players in any format
            </li>
            <li>
              Ratings are tracked separately for each format (Classical, Rapid,
              Blitz)
            </li>
