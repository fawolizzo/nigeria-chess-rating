...
const RatingTabContent: React.FC<RatingTabContentProps> = ({
  ...
  // Apply Nigerian Chess Rating logic
  const displayRating = currentRating || FLOOR_RATING;
  const displayGames = gameCount || 0;
  const ratingStatus = getRatingStatus(displayRating, displayGames);
        ...
        <Card>
            ...
            <CardTitle className="flex items-center">
              ...
              <TooltipProvider>
                <UITooltip>
                  ...
                  <TooltipContent>
                    <p>Nigerian Chess Rating System:</p>
                    <p>• Floor Rating (800): Unrated player</p>
                    <p>• Provisional: Rating &gt; 800, games &lt; 30</p>
                    <p>• Established: Rating &gt; 800, games ≥ 30</p>
                  </TooltipContent>
                </UITooltip>
              </TooltipProvider>
            </CardTitle>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <div className="text-4xl font-bold">{displayRating}</div>
              <div>
                <Badge variant={getBadgeVariant(ratingStatus)}>
                  {getStatusLabel(ratingStatus)}
                </Badge>
                <div className="text-sm text-muted-foreground mt-1">
                  {displayGames} games played
                  {ratingStatus === 'provisional' && (
                    <span className="block text-xs">
                      {PROVISIONAL_GAMES_REQUIRED - displayGames} more until
                      established
                    </span>
                  )}
                  {ratingStatus === 'unrated' && (
                    <span className="block text-xs">
                      Play {PROVISIONAL_GAMES_REQUIRED} games to get a
                      provisional rating
                    </span>
                  )}
                </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <YAxis
                      domain={[
                        (dataMin: number) =>
                          Math.max(FLOOR_RATING - 100, dataMin - 100),
                        (dataMax: number) => dataMax + 100,
                      ]}
                      tick={{ fontSize: 12 }}
                    />
                    <Tooltip
                      formatter={(value) => [`${value} points`, 'Rating']}
                    />
                    <ReferenceLine
                      y={FLOOR_RATING}
                      stroke="#FF7E67"
                ...
                <TableBody>
                  {tournamentResults.map((result, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {result.score || 'N/A'}/{result.gamesPlayed || 'N/A'}
                      </TableCell>
                      <TableCell
                        className={
