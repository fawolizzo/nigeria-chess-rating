  ...
  const processRatingChanges = async () => {
    ...
    try {
      ...
      tournament.results.forEach((result: any) => {
        const whitePlayerIndex = updatedPlayers.findIndex(
          (p: any) => p.id === result.whiteId
        );
        const blackPlayerIndex = updatedPlayers.findIndex(
          (p: any) => p.id === result.blackId
        );
        ...
        if (whitePlayerIndex !== -1 && blackPlayerIndex !== -1) {
          ...
          const whiteScore =
            result.result === '1-0' ? 1 : result.result === '0-1' ? 0 : 0.5;
