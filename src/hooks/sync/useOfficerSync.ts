    ...
    async (showToast = false) => {
      ...
      try {
        ...
        if (showToast) {
          manageToastDisplay(
            'Syncing Data',
            'Syncing dashboard data with the latest updates...'
          );
        }
        ...
        if (mountedRef.current) {
          ...
          if (showToast) {
            toast({
              title: 'Sync Error',
              description:
                'There was an error syncing the dashboard data. Please try again.',
              variant: 'destructive',
              duration: 5000,
            });
          }
