  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (data.role === 'rating_officer') {
        ...
        try {
          logMessage(
            LogLevel.INFO,
            'useLoginForm',
            'Attempting Rating Officer login with email:',
            data.email
          );
          ...
          if (success) {
            ...
            toast({
              ...
              description:
                'Welcome back! You are now logged in as a Rating Officer.',
            });
            logMessage(
              LogLevel.INFO,
              'useLoginForm',
              'Rating Officer login successful'
            );
          } else {
        ...
        try {
          logMessage(
            LogLevel.INFO,
            'useLoginForm',
            'Attempting Tournament Organizer login with email:',
            data.email
          );
          ...
          if (success) {
            logMessage(
              LogLevel.INFO,
              'useLoginForm',
              'Tournament Organizer login successful'
            );
            toast({
              title: 'Login Successful',
              description:
                'Welcome back! You are now logged in as a Tournament Organizer.',
              duration: 3000,
            });
          } else {
            throw new Error(
          ...
          logMessage(
            LogLevel.ERROR,
            'useLoginForm',
            'Tournament Organizer login error',
            {
              error: error.message,
              loginStage,
            }
      ...
      logMessage(
        LogLevel.ERROR,
        'useLoginForm',
        `Login error at stage: ${loginStage}`,
        {
          error: error.message,
          stack: error.stack,
        }
