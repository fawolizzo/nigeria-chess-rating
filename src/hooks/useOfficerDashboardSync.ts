...
export function useOfficerDashboardSync() {
  ...
  const syncDashboardData = useCallback(async (showToast = false) => {
    ...
    try {
      ...
      logMessage(LogLevel.INFO, 'useOfficerDashboardSync', 'Starting dashboard sync');
      ...
      logMessage(LogLevel.INFO, 'useOfficerDashboardSync', 'Dashboard sync completed successfully');
    } catch (error) {
