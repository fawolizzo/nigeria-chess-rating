    ...
    const cleanup = setupSyncListeners(
      ...
      () => {
        ...
        if (!isProduction) {
          toast({
            ...
            description:
              'The system has been reset from another device. The page will reload.',
            duration: 3000,
          });
        }
      ...
      async (data) => {
        if (syncInProgressRef.current) {
          console.log(
            '[SyncListeners] Sync already in progress, deferring new sync event'
          );
          return;
        }
      ...
      async (userData) => {
        console.log(
          '[SyncListeners] Login event received',
      ...
      async (userId) => {
        ...
        if (!isProduction) {
          toast({
            ...
            description:
              'Your account status has been updated. The page will refresh to apply changes.',
            duration: 4000,
          });
        }
