...
export const initializePlayerData = (playerData: any): Player => {
  ...
  return {
    ...
    ratingHistory:
      Array.isArray(playerCopy.ratingHistory) &&
      playerCopy.ratingHistory.length > 0
    ...
    rapidRating:
      playerCopy.rapidRating !== undefined
        ? playerCopy.rapidRating
        : FLOOR_RATING,
    rapidGamesPlayed: playerCopy.rapidGamesPlayed || 0,
    rapidRatingStatus:
      playerCopy.rapidGamesPlayed >= 30 ? 'established' : 'provisional',
    rapidRatingHistory:
      Array.isArray(playerCopy.rapidRatingHistory) &&
      playerCopy.rapidRatingHistory.length > 0
    ...
    blitzRating:
      playerCopy.blitzRating !== undefined
        ? playerCopy.blitzRating
        : FLOOR_RATING,
    blitzGamesPlayed: playerCopy.blitzGamesPlayed || 0,
    blitzRatingStatus:
      playerCopy.blitzGamesPlayed >= 30 ? 'established' : 'provisional',
    blitzRatingHistory:
      Array.isArray(playerCopy.blitzRatingHistory) &&
      playerCopy.blitzRatingHistory.length > 0
