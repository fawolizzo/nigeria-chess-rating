...
export function generateSwissPairings(
  ...
  console.log(
    `Generating pairings for round ${currentRound} with ${players.length} players`
  );
  ...
  const sortedPlayers = [...approvedPlayers].sort(
  ...
  previousRounds.forEach((round) => {
    round.matches.forEach((match) => {
      if (!playerData[match.whiteId] || !playerData[match.blackId]) {
        console.warn(
          `Match with invalid player IDs: ${match.whiteId} vs ${match.blackId}`
        );
        return;
      }
  ...
  sortedScores.forEach((score) => {
    ...
    console.log(
      `Processing score group ${score} with ${playersInGroup.length} players`
    );
    ...
    while (playersToPair.length >= 2) {
      ...
      for (let j = 0; j < playersToPair.length; j++) {
        ...
        if (
          playerData[playerId].opponentIds.includes(opponentId) ||
          playerId === opponentId
        ) {
          continue;
        }
      ...
      if (bestOpponentIndex >= 0) {
        ...
        if (player1ColorBalance < player2ColorBalance) {
          ...
          const player1LastColor =
            playerData[playerId].colorHistory[
              playerData[playerId].colorHistory.length - 1
            ];
  ...
  while (unpairedPlayers.length >= 2) {
    ...
    for (let i = 0; i < unpairedPlayers.length; i++) {
      ...
      const scoreDiff = Math.abs(
        playerData[playerId].score - playerData[opponentId].score
      );
      penalty += scoreDiff * 100;

      // Small penalty for rating difference
      const ratingDiff =
        Math.abs(playerData[playerId].rating - playerData[opponentId].rating) /
        100;
      penalty += ratingDiff;
    ...
    if (bestOpponentIndex >= 0 && minPenalty < 9999) {
      // Only pair if previous match penalty not triggered
      ...
      console.log(
        `Pairing (cross-score): ${approvedPlayers.find((p) => p.id === whiteId)?.name || whiteId} (W) vs ${approvedPlayers.find((p) => p.id === blackId)?.name || blackId} (B)`
      );
      pairings.push({ whiteId, blackId });
    } else {
  ...
  if (unpairedPlayers.length === 1) {
    ...
    console.log(
      `Player ${approvedPlayers.find((p) => p.id === byePlayerId)?.name || byePlayerId} gets a bye`
    );
  ...
  pairings.sort((a, b) => {
    const playerARating =
      approvedPlayers.find((p) => p.id === a.whiteId)?.rating || 0;
    const playerBRating =
      approvedPlayers.find((p) => p.id === b.whiteId)?.rating || 0;
    return playerBRating - playerARating;
  });

  console.log(
    `Generated ${pairings.length} pairings for round ${currentRound}`
  );
  return pairings;
}
 ...
 */
export const initializeStandingsByRating = (
  players: Player[]
): PlayerStanding[] => {
  return players
    .map((player) => ({
