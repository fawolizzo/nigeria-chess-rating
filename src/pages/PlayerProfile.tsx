  ...
  useEffect(() => {
    const fetchPlayerData = async () => {
      ...
      try {
        ...
        if (playerData) {
          console.log(
            '[PlayerProfile] Player data fetched successfully:',
            playerData.id
          );
          // Add mock rating history if none exists
          if (
            !playerData.ratingHistory ||
            playerData.ratingHistory.length === 0
          ) {
            playerData.ratingHistory = [
              {
          ...
          if (
            ...
            playerData.rapidRatingHistory.length === 0
          ) {
            playerData.rapidRatingHistory = [
              {
                date: new Date().toISOString(),
          ...
          if (
            ...
            playerData.blitzRatingHistory.length === 0
          ) {
            playerData.blitzRatingHistory = [
              {
                date: new Date().toISOString(),
          ...
          console.warn(
            '[PlayerProfile] Player not found with ID via Supabase:',
            id
          );
          setFetchError(
            'Player not found. The profile may not exist or the ID is incorrect.'
          );
          setPlayer(null);
          toast({
        ...
        console.error(
          '[PlayerProfile] Error fetching player from Supabase:',
          err
        );
        setFetchError(
          `Failed to load player profile: ${err.message || 'An unexpected error occurred.'}`
        );
        setPlayer(null);
        toast({
        ...
        <div className="pt-24 pb-20 px-4 sm:px-6 md:px-8 max-w-7xl mx-auto">
          ...
          <Card className="border-yellow-400 dark:border-yellow-700">
            <CardContent className="p-8 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                The player profile you are looking for does not exist or could
                not be loaded.
              </p>
              <Button onClick={handleBackToPlayers}>
                Return to Players List
