...
const runPlayerApprovalFlowTest = async (): Promise<boolean> => {
  try {
    logMessage(
      LogLevel.INFO,
      'SystemTest',
      '===== STARTING PLAYER APPROVAL FLOW TEST ====='
    );
    ...
    // Get current user
    const currentOrganizer = JSON.parse(
      localStorage.getItem('ncr_current_user') || 'null'
    );
    ...
    const allApproved = storedPlayers.every(
    ...
    logMessage(
      LogLevel.INFO,
      'SystemTest',
      '===== PLAYER APPROVAL FLOW TEST PASSED ====='
    );
    return true;
  } catch (error) {
    logMessage(
      LogLevel.ERROR,
      'SystemTest',
      '===== PLAYER APPROVAL FLOW TEST FAILED =====',
      error
    );
    await teardownSystemTest();
    return false;
