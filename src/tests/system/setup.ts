...
export const generateTestData = () => {
  ...
  return {
    ...
    testTournament: {
      ...
      description: `Description for Test Tournament ${uniqueId}`, // Added the description field
      startDate: new Date(Date.now() + 86400000).toISOString().split('T')[0], // Tomorrow as ISO string date
      endDate: new Date(Date.now() + 86400000 * 3).toISOString().split('T')[0], // 3 days later as ISO string date
      venue: 'Test Venue',
      state: 'Lagos',
      city: 'Lagos',
      rounds: 5,
...
export const teardownSystemTest = async (): Promise<void> => {
  try {
    logMessage(
      LogLevel.INFO,
      'SystemTest',
      'Tearing down system test environment'
    );
    ...
    logMessage(
      LogLevel.ERROR,
      'SystemTest',
      'Failed to tear down test environment',
      error
    );
  }
};
...
export const simulateRegistration = async (
  user: TestUser
): Promise<boolean> => {
  try {
    logMessage(
      LogLevel.INFO,
      'SystemTest',
      `Simulating registration for ${user.email} (${user.role})`
    );
    ...
    const newUser = {
      ...
      status:
        user.status ||
        (user.role === 'rating_officer' ? 'approved' : 'pending'),
    };
...
export const simulateLogin = async (
  ...
  try {
    logMessage(
      LogLevel.INFO,
      'SystemTest',
      `Simulating login for ${email} (${role})`
    );
