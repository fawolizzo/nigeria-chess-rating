...
export const createInitialRatingOfficerIfNeeded = async () => {
  try {
    ...
    logMessage(LogLevel.INFO, 'createInitialRatingOfficer', `Attempting to create/check rating officer: ${DEFAULT_RATING_OFFICER_EMAIL}`);
    ...
    if (signUpError) {
      ...
      logMessage(LogLevel.INFO, 'createInitialRatingOfficer', 'Rating officer account created successfully in Supabase');
    }

    // Try to create the tournament organizer in Supabase
    logMessage(LogLevel.INFO, 'createInitialRatingOfficer', `Attempting to create/check tournament organizer: ${DEFAULT_TOURNAMENT_ORGANIZER_EMAIL}`);
    ...
    if (orgSignUpError) {
      if (orgSignUpError.message && orgSignUpError.message.toLowerCase().includes("already registered")) {
      ...
      logMessage(LogLevel.INFO, 'createInitialRatingOfficer', 'Default tournament organizer account created successfully in Supabase');
    }
    return true;
  } catch (error) {
