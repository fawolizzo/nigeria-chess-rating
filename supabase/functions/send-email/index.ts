    ...
    while (attempt < MAX_RETRIES) {
      ...
      try {
        ...
        if (error) {
          ...
          console.log("Email sent successfully on attempt", attempt, ":", resendData);
          return new Response(
            JSON.stringify({ data: resendData, attempt }),
            { 
              status: 200, 
              headers: { ...corsHeaders, "Content-Type": "application/json" } 
            }
        ...
        console.error(`Exception during email sending (attempt ${attempt}):`, sendError);
        lastError = sendError;
        // Wait before retry (exponential backoff)
        if (attempt < MAX_RETRIES) {
          const backoffMs = Math.pow(2, attempt) * 500;
